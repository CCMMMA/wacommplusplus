...
void Particle::move(std::shared_ptr<Config> config,
    ...
    for (unsigned t=0;t<iint;t++) {
        ...
        if (oceanModelAdapter->Mask()(jI,iI)<=0.0 || jI>=eta_rho|| iI>=xi_rho) {
            health=-1;
            pstatus=0;
            return;
        }
        ...
        double u1=oceanModelAdapter->U()(ocean_time_idx,kF,jF,iF)*(1-iF)*(1-jF);
        double u2=oceanModelAdapter->U()(ocean_time_idx,kF,jF+1,iF)*(1-iF)*jF;
        double u3=oceanModelAdapter->U()(ocean_time_idx,kF,jF+1,iF+1)*iF*jF;
        double u4=oceanModelAdapter->U()(ocean_time_idx,kF,jF,iF+1)*iF*(1-jF);
        double uu=u1+u2+u3+u4;

        //printf("uu:%f\n",uu);

        double v1=oceanModelAdapter->V()(ocean_time_idx,kF,jF,iF)*(1-iF)*(1-jF);
        double v2=oceanModelAdapter->V()(ocean_time_idx,kF,jF+1,iF)*(1-iF)*jF;
        double v3=oceanModelAdapter->V()(ocean_time_idx,kF,jF+1,iF+1)*iF*jF;
        double v4=oceanModelAdapter->V()(ocean_time_idx,kF,jF,iF+1)*iF*(1-jF);
        double vv=v1+v2+v3+v4;

        //printf("vv:%f\n",vv);

        double w1=oceanModelAdapter->W()(ocean_time_idx,kF,jF,iF)*(1-iF)*(1-jF)*(1-kF);
        double w2=oceanModelAdapter->W()(ocean_time_idx,kF,jF+1,iF)*(1-iF)*jF*(1-kF);
        double w3=oceanModelAdapter->W()(ocean_time_idx,kF,jF+1,iF+1)*iF*jF*(1-kF);
        double w4=oceanModelAdapter->W()(ocean_time_idx,kF,jF,iF+1)*iF*(1-jF)*(1-kF);
        double w5=oceanModelAdapter->W()(ocean_time_idx,kF+1,jF,iF)*(1-iF)*(1-jF)*kF;
        double w6=oceanModelAdapter->W()(ocean_time_idx,kF+1,jF+1,iF)*(1-iF)*jF*kF;
        double w7=oceanModelAdapter->W()(ocean_time_idx,kF+1,jF+1,iF+1)*iF*jF*kF;
        double w8=oceanModelAdapter->W()(ocean_time_idx,kF+1,jF,iF+1)*iF*(1-jF)*kF;
        ...
        double a1=oceanModelAdapter->AKT()(ocean_time_idx,kF,jF,iF)*(1-iF)*(1-jF)*(1-kF);
        double a2=oceanModelAdapter->AKT()(ocean_time_idx,kF,jF+1,iF)*(1-iF)*jF*(1-kF);
        double a3=oceanModelAdapter->AKT()(ocean_time_idx,kF,jF+1,iF+1)*iF*jF*(1-kF);
        double a4=oceanModelAdapter->AKT()(ocean_time_idx,kF,jF,iF+1)*iF*(1-jF)*(1-kF);
        double a5=oceanModelAdapter->AKT()(ocean_time_idx,kF+1,jF,iF)*(1-iF)*(1-jF)*kF;
        double a6=oceanModelAdapter->AKT()(ocean_time_idx,kF+1,jF+1,iF)*(1-iF)*jF*kF;
        double a7=oceanModelAdapter->AKT()(ocean_time_idx,kF+1,jF+1,iF+1)*iF*jF*kF;
        double a8=oceanModelAdapter->AKT()(ocean_time_idx,kF+1,jF,iF+1)*iF*(1-jF)*kF;
        ...
        d1=pow(sin(0.5*d1),2) + pow(sin(0.5*d2),2)* cos(oceanModelAdapter->LatRad()(jI+1,iI))*cos(oceanModelAdapter->LatRad()(jI,iI));
...
std::string Particle::to_string() {
    std::stringstream ss;
    ss << "k:" << k << " j:" << j << " i:" << i << " health:" << health << " tpart:" << tpart;
    return ss.str();
}
